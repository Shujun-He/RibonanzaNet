...
train_dataset=RNADataset(train_indices,data_dict,k=config.k,use_bpp=config.use_bpp,bpp_file_folder=config.bpp_file_folder)
...
#get pl train dataset
#pl_train=pl.read_parquet(f"{config.input_dir}/merged_noisy_train_test_3parts/merged_noisy_train_test_pl_top3_rntest5.parquet")
...
#pl_train=pl.read_parquet(f"{config.input_dir}/merged_noisy_train_test_3parts/merged_on_imp.parquet")
#pl_train=pl.read_parquet(f"{config.input_dir}/top3_ensembled_unused.parquet")
...
# n=len(pl_train)//2
# np.random.seed(1)
# sampled=np.random.choice(np.arange(n),size=1_127_592//2,replace=False)
# sampled=np.stack([sampled*2,sampled*2+1]).T.reshape(-1)
# pl_train=pl_train[sampled]
# print(pl_train.shape)

#pl_train=pl.read_parquet(f"{config.input_dir}/top3_ensembled_unused.parquet")
pl_train_dict=get_pl_train(pl_train,seq_length=206)
del pl_train
pl_test_dict=get_pl_train(pl_test,seq_length=457)
del pl_test
# print(len(pl_train_dict['sequences']))
# print(len(pl_test_dict['sequences']))


for k in ['sequences','sequence_ids']:
    pl_train_dict[k]=pl_train_dict[k]+pl_test_dict[k]

for k in ['labels','errors']:
    pl_train_dict[k] = np.pad(pl_train_dict[k], ((0, 0), (0, 457-206), (0, 0)), mode='constant')
    pl_train_dict[k]=np.concatenate([pl_train_dict[k],pl_test_dict[k]])

for k in ['SN']:
    pl_train_dict[k]=np.concatenate([pl_train_dict[k],pl_test_dict[k]])
# print(len(pl_train_dict['sequences']))

pl_train_dataset=RNADataset(np.arange(len(pl_train_dict['sequences'])),pl_train_dict,k=config.k,use_bpp=config.use_bpp,bpp_file_folder=config.bpp_file_folder)
pl_train_loader=DataLoader(pl_train_dataset,batch_size=config.batch_size,shuffle=True,
                        collate_fn=Custom_Collate_Obj(),num_workers=min(config.batch_size,16))

print(len(pl_train_dataset))#,(len(train_dataset)+len(val_dataset)+len(pl_train_dataset)))
#assert n_sequences_total==(len(train_dataset)+len(val_dataset)+len(pl_train_dataset))

print(accelerator.distributed_type)
if str(accelerator.distributed_type)=="DistributedType.NO":
    use_syncbn=False
else:
    use_syncbn=True
...
model, optimizer, train_loader, val_loader, pl_train_loader, lr_schedule= accelerator.prepare(
    model, optimizer, train_loader, val_loader, pl_train_loader, lr_schedule
)
...
#SSH FS test 
    ...
    for idx,batch in enumerate(tbar):
        ...
        #bpp_flipped=bpp.clone()#.cuda().float()
        #batch_attention_mask_flipped=batch_flipped['attention_mask']
        # print(batch_attention_mask_flipped.shape)
        # print(bpp_flipped.shape)
        # print(src.shape)
        #batch_attention_mask_flipped=torch.stack([batch_attention_mask_flipped[:,:src.shape[-1],:src.shape[-1]],bpp_flipped],1)
        length=batch['length']
        ...
        for batch_idx in range(len(src)):
            ...
            #torch.stack([batch['attention_mask'][:,:src.shape[-1],:src.shape[-1]],bpp_flipped],1)
